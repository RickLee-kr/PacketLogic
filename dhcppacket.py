...
def DecodeInt(code, length, payload):
	try:
		i = int(payload[0:length])
	except:
		i = 0
	return i
...
def DecodeOption82(code, length, payload):
	...
	while i < length:
		...
		nam = ops.get(typ, "Unknown (%d)" % typ)
		if leng == 0:
			ret[nam] = "<empty>"
		else:
			ret[nam] = decodehwaddr(payload[i+2:i+leng+2])
...
def DecodePList(code, length, payload):
	...
	for i in range(0, length):
		...
		if dhcpoption.has_key(paramcode):
...
#DHCP Options and BOOTP vendor extensions
	...
	13: ('boot-file-size', DecodeInt), #int2
...
#IP Layer Parameters per Host
	...
	22: ('max-datagram-size', DecodeInt),
	23: ('default-ip-ttl', DecodeInt),
	24: ('mtu-timeout', DecodeInt),
	25: ('path-mtu-plateau-table', None), #Int2 list, to be implemented
#IP Parameters per Interface
	26: ('interface-mtu', DecodeInt),
