...
def display_view(root):
	...
	for service in root.children:
		...
		if(dl_data.has_key('quality')):
			...
			speed = dl_data['speed']
			if(quality[2] < 0.97 or quality[3] < 0.97):
				if service.name not in services:
					if(speed[0] > 1000000):
						print "%s Trigger: %s Speed: %d %d Quality: %.2f %.2f" % (datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), service.name, speed[0], speed[1], quality[2], quality[3])
						services.append(service.name)
			else:
				if service.name in services:
					print "%s Reset: %s Speed: %d %d Quality: %.2f %.2f" % (datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), service.name, speed[0], speed[1], quality[2], quality[3])
					services.remove(service.name)
				

v = rt.get_view_builder()
v.filter("Visible NetObject", "PSM")
v.distribution("AS Server Origin")
